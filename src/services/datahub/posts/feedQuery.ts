...
export const GET_FEED_POSTS = gql`
  ...
  query getFeedPosts(
    ...
    posts(
      args: {
        filter: {
          postKind: "RegularPost"
          hidden: false
          spacesFollowerAccount: $spacesFollowerAccount
        }
        pageSize: $pageSize
        offset: $offset
        orderBy: $orderBy
...
async function getFeedPostIdsRaw({
  page,
  client,
  pageSize,
  address,
  ...
  const oldIds = getFeedPostIds.getFirstPageData(client, { pageSize, address })
  ...
  >({
    document: GET_FEED_POSTS,
    variables: {
      spacesFollowerAccount: address,
...
const FEED_POST_IDS_QUERY_KEY = 'postIds'
const getFeedQueryKey = (data: HomePostsData) => [FEED_POST_IDS_QUERY_KEY, data]
export const getFeedPostIds = {
  getPaginatedQueryKey: getFeedQueryKey,
  getFirstPageData: (client: QueryClient, data: HomePostsData) => {
    const cachedData = client?.getQueryData(getFeedQueryKey(data))
  ...
  fetchFirstPageQuery: async (
    ...
    const res = await getFeedPostIdsRaw({
      ...data,
      page,
      client,
      pageSize,
    ...
    client.setQueryData(getFeedQueryKey(data), {
      pageParams: [1],
      pages: [res],
    })
    return res
  ...
  setQueryFirstPageData: (
    ...
    client.setQueryData(getFeedQueryKey(data), (oldData: any) => {
  ...
  invalidateLastQuery: (client: QueryClient, data: HomePostsData) => {
    client.refetchQueries(getFeedQueryKey(data), {
  ...
  invalidateFirstQuery: (client: QueryClient, data: HomePostsData) => {
    client.invalidateQueries(getFeedQueryKey(data), {
